            ...
            <div class="row">
                                ...
                                <div class="form-body">
                                    <div class="row">
                                        ...
                                        <div class="col-12 mt-2">
                                            <h6 class="mb-1">Test Connection</h6>
                                        </div>

                                        <div class="col-12">
                                            <div class="mb-1">
                                                <label for="test_destination" class="form-label">Test Recipient (with country code)</label>
                                                <input type="text" id="test_destination" class="form-control" value="8801000000000" name="test_destination">
                                            </div>
                                        </div>

                                        <div class="col-12">
                                            <div class="mb-1">
                                                <label for="test_sender_id" class="form-label">Test Sender ID (optional)</label>
                                                <input type="text" id="test_sender_id" class="form-control" value="{{ $data['source_value'] ?? '' }}" name="test_sender_id">
                                            </div>
                                        </div>
                ...
                <div class="col-md-8 col-12">
                    <div class="card">
                        ...
                        <div class="card-header">
                            ...
                            <div class="d-flex gap-1">
                                <button class="btn btn-outline-primary" id="btn-test-connection" type="button"><i data-feather="activity"></i> Test Connection</button>
                    ...
                    <div class="card">
                        ...
                        <div class="card-content">
                            <div class="card-body">
                                <div id="test-connection-status" class="alert alert-secondary mb-1">Click "Test Connection" to verify API and see debug output.</div>
                                <pre id="test-connection-debug" class="p-1 bg-light border rounded" style="white-space: pre-wrap; max-height: 360px; overflow: auto;">No debug output yet.</pre>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    (function () {
        ...
        const debugBox = document.getElementById('test-connection-debug');
        ...
        button.addEventListener('click', async function () {
            ...
            statusBox.className = 'alert alert-info mb-1';
            statusBox.textContent = 'Testing connection...';
            debugBox.textContent = 'Please wait...';

            const formData = new FormData(form);

            try {
                const response = await fetch("{{ route('admin.sending-servers.test.custom') }}", {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok && data.status === 'success') {
                    statusBox.className = 'alert alert-success mb-1';
                    statusBox.textContent = data.message + ' (Saved in daily log)';
                } else {
                    statusBox.className = 'alert alert-danger mb-1';
                    statusBox.textContent = (data.message || 'Connection test failed') + ' (Saved in daily log)';
                }

                debugBox.textContent = JSON.stringify(data.debug || data, null, 2);
            } catch (error) {
                statusBox.className = 'alert alert-danger mb-1';
                statusBox.textContent = 'Connection test failed: ' + error.message;
